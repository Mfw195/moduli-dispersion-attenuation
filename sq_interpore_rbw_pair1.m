...
yita=0.001*viso;% pa.s
phi=0.132;%porosity   0.0893
...
xx=0:0.2:40;
xx=xx*0.001; %pa
yy=t1(1)*(1-t1(2)*(t1(1)-Cg)*xx+t1(3)*t1(4)*exp(-t1(3)*t1(1)*xx));
...
yy3=g1(1)*(1-g1(2)*(g1(1)-Cg)*xx+g1(3)*g1(4)*exp(-g1(3)*g1(1)*xx));
%%%%%%%%%%%%%%%%%
Kh=1/Cds0;          
Fc=(5.59-Vpg)/2.18;   
Vsg=3.52-1.89*Fc;     
den_g=Kg/(Vpg^2-4*Vsg^2/3);
ug=den_g*Vsg^2; %Gpa
ug1=ug*10^9; 
sub=Kh/Kg;
syms phi_h 
...
ue=ug*(1-solution_phi_e)^(3/(1-solution_phi_e));%Gpa
alpha_m=Ke*(3*Ke+4*ue)/(pi*ue*theta_m)/(3*Ke+ue);
fc2=4*uh*(3*Kh+uh)*alpha_m^3/3/yita/(3*Kh+4*uh)*10^9;
%%%%%%%%%%%%%%%%%
Kds_p=1./Cds_p1;
Kds_p1=ones(length(omega),1)*Kds_p*10^9;
yy2=1./yy;
Kd_p1=ones(length(omega),1)*yy2*10^9;
yy4=1./yy3;
ud_p1=ones(length(omega),1)*yy4*10^9;
phi_cp=phi_co*exp(-theta_c*Cds0*xx);
Sc=0.0011;
Kmf=1./((1./Kds_p1)+(1./((1./((1./Kd_p1)-(1./Kds_p1)))+(3*1i.*(omega'*yita*(Sc.*phi_cp.^-1))/8/alpha_c^2))));
...
Es_5=Es(:,14);   
Es_15=Es(:,15);  
Es_5_new = Es_5/0.9e9-13.3 -2.0;
Es_15_new= Es_15/1e9*0.98-4.5 -2.1+0.8;
IQ_E=imag(Es)./real(Es);
IQ_E_5=IQ_E(:,6);  
IQ_E_15=IQ_E(:,2);  
IQ_E_5_new = IQ_E_5/1.4+0.032;
IQ_E_15_new= IQ_E_15./2.3+0.016;
Ks_5=Ks(:,13);
Ks_15=Ks(:,10);
Ks_5_new = Ks_5./0.6e9-19 -0.2;
Ks_15_new= Ks_15./0.6e9-23.5 -0.25;
us_5=us(:,10);
us_15=us(:,8);
us_5_new = us_5./0.8e9*0.9-6.8 -0.6+0.95;
us_15_new= us_15./1.1e9*0.91-1.7 +0.1;
...
frequencyrange2 = frequencyrange(10^4:end);
af1 = 100;    
af2 = 40;
af = 70;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
open('./Data/a_E.fig'); hold on;
    h1=semilogx(frequencyrange*1.8,Es_5_new,'-b','linewidth',1.2);     
    h2=semilogx(frequencyrange*1.8,Es_15_new,'-r','linewidth',1.2);    
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  QE   %%%%%%%%%%%%%%%%%%%%%%
open('./Data/b-QE.fig');hold on;
    semilogx(frequencyrange*4.8,IQ_E_5_new,'-b','linewidth',1.2);   
    semilogx(frequencyrange*3.3,IQ_E_15_new,'-r','linewidth',1.2);
...
open('./Data/c-K.fig');hold on;
    semilogx(frequencyrange,Ks_5_new,'-r','linewidth',1.2);  
    semilogx(frequencyrange,Ks_15_new,'-b','linewidth',1.2); 
    ...
    set(gca,'xtick',[0.01,1,10^2,10^4,10^6]);
    set(gca,'ytick',[5,10,15,20,25,30,35,40]);
    ylabel('Bulk modulus {\itK} (GPa)','Fontsize',13);
    xlabel('Apparent frequency (Hz)');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% u   %%%%%%%%%%%%%%%%%%%%%%%
open('./Data/d-u.fig');hold on;
    semilogx(frequencyrange,us_5_new,'-b','linewidth',1.2);
    semilogx(frequencyrange,us_15_new,'-r','linewidth',1.2);   
    
    semilogx(frequencyrange(13:end)*1.8*af1,Gsat(1,13:end),'-m','linewidth',1.2);
